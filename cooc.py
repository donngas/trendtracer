...
#Function to assign colors to edges based on their weights relatively
...
def get_cooc_network(file, saving_directory):
    ...
    #Handle exception during loading CSV file
    except Exception as error_while_loading:

        #Return with error statement
    ...
    try:
        ...
        print("[Cooc] Co-occurence matrix built.")
        print("[Cooc]", error_while_creating_iter, "exceptions occured during creation. Accountable rows were skipped, if any.")

    #Handle exception during creating co-oc matrix
    ...
    try:
        ...
        print("[Cooc] About to draw graph.")
        ...
        color_map = assign_colors_by_rank(sorted_nodes)
        ...
        for edge in G.edges():
            ...
            if edge_color == edge_colors_set[0]:
                line_width = 1.3
            elif edge_color == edge_colors_set[1]:
                line_width = 0.8
            else:
                line_width = 0.5
        ...
        for node in G.nodes():
            ...
            while str(node) in connected_keywords:
                connected_keywords.remove(str(node))
            connected_keywords.sort(key=lambda n: G[node][n]['weight'], reverse=True)
            connected_keywords_text = ''
            #Line break every 5 connected keywords
            ...
            #Line break every 5 connected keywords
            #for i in range(0, len(connected_keywords), 5):
                #edge_color_map[(str(node), )]
                #if i == int(len(connected_keywords)/5)*5:
                    #connected_keywords_text += ', '.join(connected_keywords[i:i + 5])
                    #break
                #connected_keywords_text += ', '.join(connected_keywords[i:i + 5]) + ',<br>'
            annotations.append(
                dict(
                    x=x, y=y,
                    xref='x1', yref='y1',
    ...
    #Handle exception during drawing
